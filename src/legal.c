...
const char *cicero_legal_move_error_to_string(int ec) {
    switch (ec) {
    case CICERO_LEGAL_MOVE:
        return "legal move";
    case CICERO_MOVE_NOT_IN_SAME_DIRECTION:
        return "move not in same direction";
    case CICERO_MOVE_LEAVES_EMPTY_SQUARES:
        return "move leaves empty squares";
    case CICERO_INVALID_ROOT_WORD:
        return "invalid root word";
    case CICERO_INVALID_CROSS_WORD:
        return "invalid cross word";
    case CICERO_NO_TILES_PLAYED:
        return "no tiles played";
    case CICERO_TOO_MANY_TILES:
        return "too many tiles played";
    case CICERO_WORD_TOO_SHORT:
        return "word is too short";
    case CICERO_FIRST_MOVE_MUST_OCCUPY_H8:
        return "first move must occupy H8";
    case CICERO_SQUARE_INVALID:
        return "invalid square";
    case CICERO_DIRECTION_INVALID:
...
int cicero_legal_move_ex(const cicero *e, const cicero_move *move,
    ...
    const dimstart vstart =
        move->direction == CICERO_HORZ ? rowstart : colstart;
    ...
    for (int i = 0; i < ntiles; ++i) {
        ...
        *p++ = tiles[i];
        for (int sq = root + stride; sq < stop && board[sq] != EMPTY;
             sq += stride) {
            *p++ = to_uppercase(to_ext(board[sq]));
        }
        *p++ = '\0';
