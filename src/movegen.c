...
internal int calc_cached_score(int start, int stop, int stride, int root,
                               const cicero *engine) {
    ...
    int partial_score = 0;
    // walk left
    for (int sq = root - stride; sq >= start && vals[sq] != EMPTY;
         sq -= stride) {
        ++tiles;
...
internal int build_word(const char *vals, const int start, const int stop,
                        const int stride, const int root, string *word) {
    ...
    // TRACE("\tbuild_word(%s) stopping on %s (sq=%d stop=%d vals[sq]='%c')",
    // SQNAME(root), SQNAME(sq), sq, stop, to_ext(vals[sq]));
    word->buf[word->len] = '\0';
...
int cicero_make_move(cicero *e, cicero_savepos *sp, const cicero_move *move) {
    ...
    for (int tidx = 0; tidx < ntiles; ++tidx) {
        ...
        if (before >= start) {
            ...
            string word;
            const int idx =
                build_word(vals, start, stop, stride, before, &word);
    ...
    {  // update vertical cross-checks
        ...
        assert(getdim(stride, lsq) ==
               getdim(stride, rsq));  // move must be on exactly 1 row or col
        if (before >= start) {
            assert(vals[before] == EMPTY);
        ...
        if (vals[sq] == EMPTY) {
            ...
            if (word.len != 1) {
                ...
                for (char c = 'A'; c <= 'Z'; ++c) {
                    word.buf[index] = c;
                    cicero_edges edges =
                        e->cb.getedges((void *)e->cb.getedgesdata, word.buf);
                    if (edges.terminal) {
                        xchk |= tilemask(tilenum(c));
                    }
                }
...
// Definitions:
//   + "Left Part"  -- prefix that goes BEFORE the anchor square, it may be
//   empty
//   + "Right Part" -- rest of word, which must start at the anchor square
//
// Therefore:
//   + Potential left part is the minimum of the distance (going left) to the
//   edge
...
//
// Additional Notes:
//   + Everything in this file is specified from the standpoint of generating
//   horizontal
...
internal void extend_right(const state *ss, int lsq, int sq, string *word,
                           int right_part_length) {
    word->buf[word->len] = 0;
    const cicero *e = ss->e;
    int *rack = ss->r->tiles;
    const cicero_edges edges_ =
        e->cb.getedges((void *)e->cb.getedgesdata, word->buf);
    ...
    if (sq >= stop || vals[sq] == EMPTY) {
        if (/*right_part_length > 0*/ sq > anchor && terminal) {
            ...
            e->cb.onlegal((void *)e->cb.onlegaldata, word->buf, lsq,
                          ss->stride);
        }
        ...
        // NOTE: need to run a second time with blanks so I check both path of
        // using
        ...
        if (rack[BLANK] > 0) {
            for (const char *tile = edges; *tile != 0; ++tile) {
                ...
                word->buf[word->len++] =
                    'a' + (*tile - 'A');   // TODO: hoist len incr out of loop
                word->buf[word->len] = 0;  // TODO: hoist out of loop
                assert(word->len <= DIM);
                extend_right(ss, lsq, nextsq, word, right_part_length + 1);
                word->len--;               // TODO: hoist out of loop
        ...
        const char tile =
            vals[sq] < BLANK
                ? vals[sq] + 'A'
                : vals[sq] - BLANK + 'A';  // force to be in range [A..Z]
...
// TODO: remove `sq` parameter, can calculate it from sq = anchor -
// strlen(word->buf) - 1 (see line 278 assertion below)
internal void left_part(const state *ss, int sq, int limit, string *word) {
    ...
    for (const char *tile = edges; *tile != 0; ++tile) {
        ...
        assert(sq >= start);
        left_part(ss, sq - stride, limit - 1,
                  word);  // try to expand the left part more to the left
        word->len--;      // TODO: hoist out of loop
        word->buf[word->len] = 0;  // TODO: hoist this out of loop
        rack[tint]++;
    }
    if (rack[BLANK] > 0) {
        for (const char *tile = edges; *tile != 0; ++tile) {
            assert(xchk[sq] ==
                   ANYTILE);  // see section 3.3.1 Placing Left Parts
            rack[BLANK]--;    // TODO: hoist out of loop
            word->buf[word->len++] =
                'a' + (*tile - 'A');   // TODO: hoist len incr out of loop
            word->buf[word->len] = 0;  // TODO: hoist this out of loop
...
internal void extend_right_on_existing_left_part(const state *ss, int anchor,
                                                 string *word) {
    const cicero *e = ss->e;
    const char *vals = e->vals;
    const int start = ss->start;
    ...
    assert(vals[sq] != EMPTY);
    // TODO: more efficient way to extend backwards? maybe make the buffer
    // (DIM+1)*2 so we can start
    //       in the middle?
    ...
    extend_right(ss, anchor - stride * word->len, anchor, word,
                 /*right_part_length*/ 0);
    word->len = 0;
}
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        while (msk > 0) {
            ...
            for (int i = 0; i < ASIZE(dirs); ++i) {
                ...
                const int start =
                    dirs[i] == HORZ ? colstart(anchor) : rowstart(anchor);
                ss.anchor = anchor;
                ss.start = start;
                ss.stride = stride;
                ss.stop = start + DIM * stride;
                ss.xchk = dirs[i] == HORZ ? e->hchk : e->vchk;
                if (anchor - stride >= start &&
                    vals[anchor - stride] != EMPTY) {
